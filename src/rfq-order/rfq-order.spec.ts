...
it('should create RfqOrder with allowed sender', () => {
    const sender = Address.fromBigInt(99n)
    const order = new RfqOrder(
        {
            makerAsset: new Address(
                '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
    ...
    expect(order.makerTraits.isPrivate()).toEqual(true)
    expect(order.makerTraits.allowedSender()).toEqual(
        sender.toString().slice(-20)
    )
})
